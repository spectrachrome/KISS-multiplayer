...
local function update_transform_info(_we_own_this_vehicle)
  ...
  -- A/B TEST SCAFFOLDING (not for upstream): the legacy fields below are the
  -- pre-motion-sync wire contents - refnode-anchored pose and raw body gyro,
  -- read fresh rather than from the physics-step sample. Published alongside
  -- the COG sample so the GE side can switch paths without a reload.
  local legacy_rot = quat(obj:getRotation())
  local legacy_pos = obj:getPosition()
  ...
  local transform = {
    ...
    legacy_position = {legacy_pos.x, legacy_pos.y, legacy_pos.z},
    legacy_rotation = {legacy_rot.x, legacy_rot.y, legacy_rot.z, legacy_rot.w},
    legacy_velocity = {obj:getVelocity().x, obj:getVelocity().y, obj:getVelocity().z},
