...
-- in a constant-acceleration extrapolation model so the consumer chases
-- where the sender IS NOW, not where the sender WAS when the packet was
-- sent.
--
local M = {}
...
-- Clamp (m/s^2) on acceleration derived from packet velocity deltas. A lost or
-- reordered packet shortens the divisor and turns an ordinary velocity change
-- into an enormous spike, which the 0.5*a*t^2 term would throw metres.
-- 100 m/s^2 is roughly 10 g: above any real vehicle, below the spike range.
local MAX_LINEAR_ACCELERATION = 100
-- Ceiling (s) on how far ahead we extrapolate. Past this the quadratic term
-- dominates and the prediction leaves the road; better to lag than to invent.
local MAX_PREDICT = 0.3
-- Stop correcting if no packet arrived within this many seconds. Freezing the
-- replica is preferable to extrapolating a stream that has stopped.
local PACKET_TIMEOUT = 0.1
-- Gap (s) after which the next packet is treated as a fresh start rather than
-- a continuation. Deriving acceleration across a multi-second gap gives a huge
-- divisor, a meaningless result, and carries stale state forward.
local STALE_THRESHOLD = 2.0
-- Rate (Hz) at which the smoothed velocity tracks the packet velocity. Low,
-- because this is the receiver's only defence against packet-to-packet noise
-- in the quantity that drives both position and derived acceleration.
-- Live-tuned.
local REMOTE_VEL_SMOOTH_RATE = 2.0
-- Rate (Hz) for the rotation smoother. Much higher than the velocity rate:
-- heading lag is immediately visible as late turn-in, so rotation is allowed
-- to track packets far more aggressively than velocity. Live-tuned.
local REMOTE_ROTATION_SMOOTH_RATE = 8.0
-- Rate (Hz) for the derived-acceleration smoother. Lower still, because
-- acceleration is a difference of an already-noisy quantity.
local REMOTE_ACCELERATION_SMOOTH_RATE = 1.0
-- Rate (Hz) at which the packet-age estimate converges. Latency changes are
-- real but slow; reacting to each packet's apparent age would modulate the
-- prediction horizon at packet rate and show up as jitter.
local TIME_OFFSET_SMOOTH_RATE = 1.0
-- Orientation error (rad) beyond which the rotation smoother snaps instead of
-- slerping. 35 degrees is far past anything a lost packet explains, so it
-- means the replica is genuinely somewhere else and smoothing would only
-- draw out a wrong pose.
...
-- Smoothing is applied per unit of prediction horizon rather than per frame:
-- the further ahead we extrapolate, the more the smoothers are allowed to
-- move, so behaviour stays comparable across different latencies.
...
local function extrapolate_transform(state, current_time)
  ...
  local predict_time = math.max(
    -MAX_PREDICT,
    math.min(current_time - calc_local_time, MAX_PREDICT)
  )
  ...
  if frame_dt > 0 then
    ...
    state.smooth_velocity = vec3_lerp(
      ...
      math.min(REMOTE_VEL_SMOOTH_RATE * smooth_dt, 1.0)
    )
  ...
  if REMOTE_ROTATION_SMOOTH_RATE > 0 then
    if not state.smooth_rotation then
      ...
      elseif frame_dt > 0 then
        state.smooth_rotation = slerp_quaternion(
          ...
          predicted_rotation,
          math.min(REMOTE_ROTATION_SMOOTH_RATE * frame_dt, 1.0)
        )
      end
    end
...
-- Drop every smoothed quantity but keep the latest snapshot. Used after a
-- discontinuity (teleport, recovery snap) where the history the smoothers
-- accumulated no longer describes the same motion.
